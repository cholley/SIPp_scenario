...
sudo sipp imsdomain.com -sf register.xml -inf $inf -i `ifconfig | head -2 | tail -1 | awk '{print $2}'` -p 15060 -r $call -rp $TIME -m $total   -timeout $TIME_OUT -timeout_error -recv_timeout $(($TIME_OUT*1000))  -send_timeout $(($TIME_OUT*1000))
sudo sipp imsdomain.com -sf sippcall.xml -inf $inf -i `ifconfig | head -2 | tail -1 | awk '{print $2}'` -p 15060 -r $call -rp $TIME -m $total  -s $SERVICE -timeout $TIME_OUT -timeout_error -recv_timeout $(($TIME_OUT*1000))  -send_timeout $(($TIME_OUT*1000))   | tee /tmp/salam.log
...
OKringing=`cat /tmp/salam.log |head -23 | tail -11 | awk '{print $4}' |head -6 |tail -1`
